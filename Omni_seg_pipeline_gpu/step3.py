...
def test(downsample_rate, now_task, pred_image, nt, img, padding_size, stride_size, output_dir, case_name, patch_size):
...
def slice_merging(big_slice_folder, output_dir, case_name, padding_size):
    ...
    for nt in range(len(task_list)):
        ...
        print(now_task)
    #
    #     pool.apply(test, args=(downsample_rate, now_task, pred_image, nt, img, padding_size,
    #                            stride_size, output_dir, case_name, patch_size))
...
def slice_merging_cpu(big_slice_folder, output_dir, case_name, padding_size):
    ...
    for nt in range(len(task_list)):
        ...
        pred_image_resize = resize(now_pred_image, (int(pred_image.shape[0] / 4), int(pred_image.shape[1] / 4), 3), anti_aliasing=False)
...
def slice_merging_mapping(big_slice_folder, output_dir, case_name, scale, padding_size):
            ...
            if ele < 0:
                ...
                for x in range(0, row, patch_x):
                    for y in range(0, col, patch_y):
                        ...
                        tmp = tmp + (big_pred[x:x + patch_x, y:y + patch_y, index] - pred[
                                                                                     x:x + patch_x,
                                                                                     y:y + patch_y,
                ...
                for x in range(0, row, patch_x):
                    for y in range(0, col, patch_y):
                        ...
                        tmp = torch.zeros(patch_x, patch_y).to(torch.int16).cuda()
                        tmp = tmp + (big_pred[x:x + patch_x, y:y + patch_y, index].to(torch.int16) + pred[
                                                                                                     x:x + patch_x,
                                                                                                     y:y + patch_y,
                                                                                                     index].to(torch.int16) / 6).to(torch.int16)
                        tmp[tmp > 255] = 255
                        tmp[tmp < 0] = 0
    ...
    for x in range(0, row, patch_x):
        for y in range(0, col, patch_y):
            ...
            big_pred[x:x + patch_x, y:y + patch_y, :] = big_pred[x:x + patch_x, y:y + patch_y, :] + image[x:x + patch_x, y:y + patch_y, :]
            ...
            check_pred[x:x + patch_x, y:y + patch_y, 0] = check_pred[x:x + patch_x, y:y + patch_y, 0] * colormap[-1][0] / 6
            check_pred[x:x + patch_x, y:y + patch_y, 1] = check_pred[x:x + patch_x, y:y + patch_y, 1] * colormap[-1][1] / 6
            check_pred[x:x + patch_x, y:y + patch_y, 2] = check_pred[x:x + patch_x, y:y + patch_y, 2] * colormap[-1][2] / 6

            check_pred[x:x + patch_x, y:y + patch_y, :] = check_pred[x:x + patch_x, y:y + patch_y, :] + big_pred[x:x + patch_x, y:y + patch_y, :]
